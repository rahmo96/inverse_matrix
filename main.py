...
def free_num(scalar,free_numbers,scalar_index,index,matrix):    #hold the value of the variables

        free_numbers[index]=(free_numbers[index]+scalar*free_numbers[scalar_index])/matrix[index,index]
        return free_numbers
...
def inverse(matrix,free_numbers):
    ...
    for i in range(n):
        ...
        for j in range(n):
            if i != j:
                ...
                matrix = np.dot(elementary_matrix, matrix)
                free_num(scalar, free_numbers, i, j,matrix)
                print(f"The matrix after elementary operation :\n {matrix}")
                print( "------------------------------------------------------------------------------------------------------------------",
                      )
    ...
    for i in range(len(free_numbers)):
        print("Variable number "+str(i+1)+"= "+ str(free_numbers[i]))
